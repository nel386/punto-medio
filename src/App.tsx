        ...
        <section className="page setup-page">
          ...
          <div id="game-settings" className="settings-grid">
            <div className="settings-main">
              ...
              <div className="teams-block"><div className="inline-heading"><div><p className="eyebrow">04 · Montad los equipos</p><h2>¿Quién juega?</h2></div><span className="selection-note">2–6 equipos</span></div><div className="team-inputs">{teamNames.map((name, index) => <label key={index} className="team-input"><span className={`team-dot team-dot-${index % 4}`} />Equipo {index + 1}<input value={name} onChange={(event) => setTeamNames((current) => current.map((item, i) => i === index ? event.target.value : item))} /></label>)}{teamNames.length < 6 && <button className="add-team" onClick={() => setTeamNames((current) => [...current, `Equipo ${current.length + 1}`])}><Plus aria-hidden="true" /> Añadir equipo</button>}{teamNames.length > 2 && <button className="remove-team" onClick={() => setTeamNames((current) => current.slice(0, -1))}><Minus aria-hidden="true" /> Quitar</button>}</div><div className="rounds-row"><span>Vueltas por equipo</span><div className="stepper"><button aria-label="Menos vueltas" onClick={() => setRoundsPerTeam((value) => Math.max(1, value - 1))}><Minus aria-hidden="true" /></button><strong>{roundsPerTeam}</strong><button aria-label="Más vueltas" onClick={() => setRoundsPerTeam((value) => Math.min(2, value + 1))}><Plus aria-hidden="true" /></button></div><span className="total-rounds">{teamNames.length * roundsPerTeam} rondas en total</span></div></div>
            </div>
